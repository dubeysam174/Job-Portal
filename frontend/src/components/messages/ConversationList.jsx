...
export default function ConversationList() {
  ...
  useEffect(() => {
    ...
    axios.get("http://localhost:8000/api/v1/conversation", {
  ...
  const openConversation = async (conversation) => {
    ...
    const res = await axios.get(
      `http://localhost:8000/api/v1/message/${conversation._id}`,
      { withCredentials: true }
    );
    if (res.data.success) dispatch(setMessages(res.data.messages));

    await axios.put(
      `http://localhost:8000/api/v1/message/seen/${conversation._id}`,
      {},
      { withCredentials: true }
    );
  };
